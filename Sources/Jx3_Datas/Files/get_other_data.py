        ...
        def wrapper(*args, **kwargs):
            ...
            with open(r'Sources/Settings/personal_data', 'a', encoding='utf-8') as f:
                ...
                if ret is not None:
                    ...
                    elif _id in personal_data[_location]:
...
def get_buff_or_skill_from_jx3box(subtype: Literal["buff", "skill"], id: int, level: int) -> dict:
    ...
    ret = {'ID': id, 'Data': {level: {'Name': f'未知{subtype}', 'BelongKungFu': -1}}}

    @write_in(subtype)
    def _get():
        nonlocal ret
        print(f'not found id = {id} {subtype}!')
        resp = requests.get(url=url, timeout=3)
        if resp.status_code == 200:
            try:
            ...
            if data is not None:
                ...
                if subtype == 'skill':
                    ret['Data'][level]['BelongKungfu'] = data['BelongKungfu']
            return ret
